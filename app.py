...
def main() -> None:
    ...
    if gamma_by_nucleus:
            ...
            json.dumps(
                {
                    ...
                    "total_deducted": gamma_total,
                    "cost_brl_per_credit": GAMMA_COST_BRL_PER_CREDIT,
                    "total_cost_brl": round(
                        gamma_total * GAMMA_COST_BRL_PER_CREDIT, 2
                    ),
                    "by_nucleus": gamma_by_nucleus,
        ...
        log.info(f"[Gamma] Creditos deduzidos no curso: {gamma_total}")
        log.info(
            f"[Gamma] Custo estimado no curso: R$ {gamma_total * GAMMA_COST_BRL_PER_CREDIT:.2f}"
        )
    ...
    if openai_by_nucleus:
        ...
        log.info(f"[OpenAI] Custo estimado no curso: {openai_total:.6f} USD")
    ...
    for entry in nuclei:
        ...
        copied += 1
    log.info(f"[dist] Slides copiados: {copied}")
